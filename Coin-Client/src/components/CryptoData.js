...
function CryptoData({name, setUpdate, portCoins, setPortCoins, user, price,dodChg, mktCap, mktCapRank, id, maxSupply, volume, index, image, round}) {
// formatting numbers.
const [clicked, setClick] = useState(false)

// had to create a new formatted version, as re-rendering for portfolio was breaking numbers.
let tableMaxSupply
let tableMarketCap
...
// IF THE THING IS IN PORTFOLIO, THIS SHOULD LOAD AS CLICKED
// a lookup to say if the above, then set clicked = true
// if(portCoins.filter(e=> e.coin_name === name).length === 0){
//   setClick(true)}
function handleClick(e){
  // CHANGE THIS TO CHECK FOR "if its not in the portfolio" instead.
  if(portCoins.filter(e=> e.coin_name === name).length === 0){
    ...
    // pass this clicked state up to the crypto list / page thing to see if u can render stars
    setClick(true)  
  ...
  // what do you do if its already in the array
else console.log('allegedly in there')
}

return (      
    <tr className="hover:bg-white">
      <td className="px-1 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
      <button onClick={handleClick}>{portCoins?portCoins.filter(e=> e.coin_name === name).length === 0?<svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
